...
def evalb(evalb_dir, gold_trees, predicted_trees, ref_gold_path=None):
    ...
    # fscore.recall = 0 if gold == 0 else 100 * match_a / gold
    # fscore.precision = 0 if pred == 0 else 100 * match_a / pred
    # fscore.fscore = 2 * fscore.precision * fscore.recall / (fscore.precision + fscore.recall) if (fscore.precision + fscore.recall) > 0 else 0
    success = (
