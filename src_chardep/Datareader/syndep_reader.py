...
def read_syndep(syndep_path, tr_max_len=0):
    ...
    while inst is not None:
        ...
        max_len = max(max_len, inst_size)
        if tr_max_len > 0 and inst_size - 1 > tr_max_len:
            inst = dep_reader.getNext()
            continue
    ...
    print("Max len of data: %d" % max_len)

    return dep_sentences, dep_pos, dep_heads, dep_types