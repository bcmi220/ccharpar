...
class InternalTreebankNode(TreebankNode):
    ...
    def pred_linearize(self):
        return "({} {})".format(
            self.label, " ".join(child.pred_linearize() for child in self.children))
...
class LeafTreebankNode(TreebankNode):
    ...
    def pred_linearize(self):
        return "({} {})".format(self.goldtag, self.word)
            ...
            if tokens[index] == "(":
                ...
                if label != '-NONE-':
                    ...
                    if goldpos is not None:
                        if len(goldpos[cun_sent]) < cun_word + 1:
                            print(len(goldpos[cun_sent]), cun_word)
                            print(len(goldpos), cun_sent)
                            print(cun_sent, cun_word, word)
                            print(cun_sent, cun_word + 1, word, heads[cun_sent][cun_word], types[cun_sent][cun_word], goldpos[cun_sent][cun_word])
